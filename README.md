...
rmdir 함수는 <unistd.h> 헤더 파일에 선언되어 있습니다. 이 함수는 인수로 전달된 디렉토리를 제거합니다.
  
main 함수의 인수 argc는 프로그램이 실행될 때 전달되는 명령줄 인수의 개수를 나타냅니다. argv는 명령줄 인수들을 저장하는 문자열 배열입니다.
  
프로그램이 실행될 때 전달된 명령줄 인수의 개수가 2개가 아닌 경우, 즉 <실행파일> <디렉토리> 형태로 입력되지 않은 경우, 사용법을 출력하고 프로그램을 종료합니다.
  
rmdir 함수를 사용하여 인수로 전달된 디렉토리를 제거합니다. 제거에 성공하면 0을 반환하고, 실패하면 -1을 반환합니다.
  
제거에 실패한 경우, perror 함수를 사용하여 rmdir 함수에서 발생한 에러 메시지를 출력합니다.
  
제거에 성공한 경우, "Directory removed successfully." 메시지를 출력하고 프로그램을 종료합니다.
